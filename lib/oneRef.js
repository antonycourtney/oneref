...
var Ref = (function (_EventEmitter) {
  ...
  /**
   * A function that, given a ref, returns an 'updater' function.
   *
   * The updater is a function that takes an ('a -> 'a) function (uf),
   * applies it to the current value in ref and stores the result
   * back in ref (which will notify registered listeners).
   *
   * We use Currying here so that we can partially apply refUpdater
   * to obtain an updater function that is passed to actions. 
   */
